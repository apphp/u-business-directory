    ...
    */
    public static function login($strategy = 'facebook', $exit = true)
    {
        ...
        if(in_array($strategy, $nameStrategies)){
            ...
            $url = A::app()->getRequest()->getBaseUrl().'social/oauth/type/'.$strategy;
            header("Location: $url");
            if($exit){
                exit();
            }
     ...
     * @param array $links
     */
    public static function drawButtons($links = array())
    {
        $output  = '';
        ...
        if(!empty($activeSocial) && is_array($activeSocial)){
            ...
            foreach($activeSocial as $social){
                $link = isset($links[$social['type']]) ? $links[$social['type']] : 'social/oauth/type/'.$social['type'];
                $output .= CHtml::tag('a', array('href'=>$link, 'class'=>'btn btn-block btn-social btn-'.$social['type'], 'type'=>'button'), '<i class="fa fa-'.CHtml::encode($social['type']).'" aria-hidden="true"></i><span class="text"> '.A::t('app', 'Signup with').' '.$social['name'].'</span>');
            }
