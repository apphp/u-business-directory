<?php
	...
	if($parentId){
		...
		$breadCrumbs[] = array('label'=>A::t('directory', 'Add Location'));
	}else{
		$breadCrumbs[] = array('label'=>A::t('directory', 'Add Sub-Location'));
	}
...
<div class="bloc">
	<?php
		...
		if($parentId){
			...
			echo '<a class="sub-tab active" href="regions/manage/parentId/'.(int)$parentId.'">'.$parentName.'</a>'.' '.A::t('directory', 'Add Sub-Location');
			echo '</div>';
		}else{
			echo '<div class="sub-title">'.A::t('directory', 'Add Location').'</div>';
		}
	?>
    ...
    <div class="content">
        <?php
			...
			CWidget::create('CDataForm', array(
				'model'			=>'Regions',
				'operationType'	=>'add',
				'action'		=>'regions/add/parentId/'.$parentId,
