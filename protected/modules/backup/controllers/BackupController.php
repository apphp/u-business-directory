...
{	
	...
	public function __construct()
	{
        ...
        if(!Modules::model()->exists("code = 'backup' AND is_installed = 1")){        
	...
	{		
		...
		if($cRequest->getPost('act') == 'send'){
            ...
            // Check filename validity
            }else if(!CValidator::isFileName($this->_view->backupFileName)){
				...
				if(APPHP_MODE == 'demo'){
					...
					$msgType = 'warning';
				}else if($this->_create($this->_view->backupFileName)){                    
    ...
    public function deleteAction($file = '')
    {
		...
		if(APPHP_MODE == 'demo'){
			$msg = A::t('core', 'This operation is blocked in Demo Mode!');
			$msgType = 'warning';
		}else if(CFile::deleteFile($backupFile)){
